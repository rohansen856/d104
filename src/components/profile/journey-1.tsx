...
export function Journey(){
    return(
        <section className="mt-6 flex min-h-[80vh] w-full flex-col items-center">
            <h3 className="mb-12 w-full py-3 text-center text-3xl font-bold lg:text-4xl">Qualifications</h3>
            <div className="flex w-full grow justify-center">
                {qualification
                    ?<div className="w-[49%] max-w-[300px]">
                        {
                            qualification.map(i=>(i.serial%2!==0)?(
                                <div className="h-[95px] w-full pr-8 text-right">
                                    <h4 className="text-xl">{i.name}</h4>
                                    <p className="text-lg">{i.place}</p>
                                    <p className="text-lg text-muted">{i.date}</p>
                                </div>
                                ):(
